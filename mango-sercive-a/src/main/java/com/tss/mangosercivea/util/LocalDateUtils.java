...
/**
 * Java8时间工具类
 * Created by yangxiangjun on 2020/12/16.
 */
public class LocalDateUtils {
    ...
    public static void main(String[] args) {
        ...
        userDTO.setId(121L);
        userDTO.setUsername("bank");
        userDTO1.setPassword("123");
        for (int i = 0;i<5000000;i++) {
            CopyUtil.copyProperties(userDTO, userDTO1);
//            methodAccess.invoke(userDTO,"getId");
        }
        long m2 = System.currentTimeMillis();
        System.out.println(m2 - m1);

        for (int i = 0;i<5000000;i++) {
            CopyUtil.copyProperties(userDTO, userDTO1,null,true,null);
        }
    ...
    private volatile static LocalDateUtils lazyMen;

    private static LocalDateUtils getInstance(){//创建一个静态方法,有返回值的

        if (lazyMen==null){
            synchronized (LocalDateUtils.class){
                if (lazyMen==null){//加了一个判断如果lazyMen是空的情况下,再去new对象
                    lazyMen=new LocalDateUtils();
                }
            }
        }
        return lazyMen;
